package com.example.sandeep.javaexosia.ViewAdaptor;
...
import com.example.sandeep.javaexosia.flashscreen;
...
import com.example.sandeep.javaexosia.UserDataCollection;
import com.example.sandeep.javaexosia.UserSection.Collectionclass.PersonSchemeCollection;
...
public class SchemeViewAdaptor extends RecyclerView.Adapter<SchemeViewAdaptor.Viewholder> {
    ...
    public SchemeViewAdaptor(Context context, ArrayList< SchemedataCollection> schemeArrayList, String id, String department) {
        this.context = context;
        this.schemeArrayList = schemeArrayList;
        this.id=id;
        dep=department;
    ...
    @Override
    public void onBindViewHolder(@NonNull @NotNull SchemeViewAdaptor.Viewholder holder, @SuppressLint("RecyclerView") int position) {
        ...
        if (dep.equals("Admin")) {
            ...
            holder.btnapply.setText("Delete");

            holder.btnapply.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {

                    reference1.orderByChild("strschName").equalTo(collection.getStrschName()).addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                            ...
                            for (DataSnapshot data:snapshot.getChildren()){
                                data.getRef().removeValue().addOnCompleteListener(new OnCompleteListener< Void >() {
                                    ...
                                    public void onComplete(@NonNull Task< Void > task) {
                                        schemeArrayList.remove(schemeArrayList.get(position));
...
//                                if (income>=in){
    ...
    private void InsertPersonalScheme(PersonSchemeCollection DataCollection) {
        ...
        Toast.makeText(context.getApplicationContext(), "Succefully Apply", Toast.LENGTH_SHORT).show();
    }
